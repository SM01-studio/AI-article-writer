        ...
        try {
            ...
            if (task.status === 'completed') {
                ...
                if (task.result && task.result.research_data) {
                    state.researchData = task.result.research_data;  
    ...
    if (state.useRealApi && state.sessionId && window.api) {
        try {
            ...
            if (result.success) {
                ...
                state.draftContent = result.data.content || result.data;  
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/chat`, {
            ...
            body: JSON.stringify({
                ...
                phase: phase,
                message: message,
                history: state.chatHistory[phase].slice(-10)  
            })
        });
...
function renderResearchFromApi(data) {
    ...
    const confirmedSources = data.confirmed_sources || [];  
    const pendingSources = data.pending_sources || data.sources || [];  
